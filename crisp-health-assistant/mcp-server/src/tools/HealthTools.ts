...
export class MCPHealthTools {
  constructor(
    private googleFitService: GoogleFitService,
    private openAIService: OpenAIService
  ) {}

  async getHealthData(metricType: string = 'all', days: number = 7): Promise<MCPToolResult> {
    try {
      let data;
  ...
  async askHealthQuestion(question: string, includeData: boolean = true): Promise<MCPToolResult> {
    try {
      let healthData;
  ...
  async getHealthSummary(period: 'today' | 'week' | 'month' = 'week'): Promise<MCPToolResult> {
    try {
  ...
  async getHealthTrends(days: number = 30): Promise<MCPToolResult> {
    try {
  ...
  private calculateAverages(healthData: any) {
    return {
      steps: Math.round(
        healthData.steps.reduce((sum: number, m: any) => sum + m.value, 0) / healthData.steps.length || 0
      ),
      calories: Math.round(
        healthData.calories.reduce((sum: number, m: any) => sum + m.value, 0) / healthData.calories.length || 0
      ),
      heartRate: Math.round(
        healthData.heart_rate.reduce((sum: number, m: any) => sum + m.value, 0) / healthData.heart_rate.length || 0
      ),
      sleep: Math.round(
        (healthData.sleep.reduce((sum: number, m: any) => sum + m.value, 0) / healthData.sleep.length || 0) * 10
      ) / 10
    };
  }

  private calculateTotals(healthData: any) {
    return {
      steps: healthData.steps.reduce((sum: number, m: any) => sum + m.value, 0),
      calories: healthData.calories.reduce((sum: number, m: any) => sum + m.value, 0),
      sleepHours: healthData.sleep.reduce((sum: number, m: any) => sum + m.value, 0)
    };
  }
}
