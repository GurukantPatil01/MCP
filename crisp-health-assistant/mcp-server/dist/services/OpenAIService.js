...
class OpenAIService {
    ...
    async askHealthQuestion(question, healthData) {
    ...
    getMockHealthResponse(question, healthData) {
    ...
    async generateHealthSummary(healthData, period) {
        // Return mock response if no API key
        if (!this.hasApiKey || !this.openai) {
    ...
    async analyzeHealthTrends(currentData, previousData) {
        if (!previousData) {
    ...
    calculateTrends(healthData) {
        const avgSteps = Math.round(healthData.steps.reduce((sum, m) => sum + m.value, 0) / healthData.steps.length || 0);
        const avgCalories = Math.round(healthData.calories.reduce((sum, m) => sum + m.value, 0) / healthData.calories.length || 0);
        const avgHeartRate = Math.round(healthData.heart_rate.reduce((sum, m) => sum + m.value, 0) / healthData.heart_rate.length || 0);
